...
class RPMPublisherLeft(Node):

    def __init__(self):
        super().__init__('rpm_publisher_left')
        self.subscription = self.create_subscription(TickCount, 'encoder_ticks', self.listener_callback, 10)
        self.subscription  # prevent unused variable warning

        self.publisher_ = self.create_publisher(WheelRPM, 'feedback_rpm_left', 10)
    ...
    def listener_callback(self, msg):
        self.ticks_current = msg.left_wheel_tick_count
...
def main(args=None):
    print('Publishing feedback_rpm_left...')
    rclpy.init(args=args)
    rpm_publisher_left = RPMPublisherLeft()
    rclpy.spin(rpm_publisher_left)

    # Destroy the node explicitly
    # (optional - otherwise it will be done automatically
    # when the garbage collector destroys the node object)
    rpm_publisher_left.destroy_node()
    rclpy.shutdown()
