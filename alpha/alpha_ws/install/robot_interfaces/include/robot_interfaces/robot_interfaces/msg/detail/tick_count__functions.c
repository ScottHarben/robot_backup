  ...
  robot_interfaces__msg__TickCount__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    robot_interfaces__msg__TickCount * data =
      (robot_interfaces__msg__TickCount *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!robot_interfaces__msg__TickCount__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          robot_interfaces__msg__TickCount__fini(&output->data[i]);
        }
        return false;
      }
    }
