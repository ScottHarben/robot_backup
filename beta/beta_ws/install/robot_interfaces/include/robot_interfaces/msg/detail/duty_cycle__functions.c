  ...
  robot_interfaces__msg__DutyCycle__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    robot_interfaces__msg__DutyCycle * data =
      (robot_interfaces__msg__DutyCycle *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!robot_interfaces__msg__DutyCycle__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          robot_interfaces__msg__DutyCycle__fini(&data[i]);
        }
        free(data);
        return false;
      }
