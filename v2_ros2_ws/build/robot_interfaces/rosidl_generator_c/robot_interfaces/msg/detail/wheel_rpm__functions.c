  ...
  robot_interfaces__msg__WheelRPM__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    robot_interfaces__msg__WheelRPM * data =
      (robot_interfaces__msg__WheelRPM *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!robot_interfaces__msg__WheelRPM__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          robot_interfaces__msg__WheelRPM__fini(&data[i]);
        }
        free(data);
        return false;
      }
