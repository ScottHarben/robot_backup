...
class T1ServoController(Node):

    def __init__(self):
        super().__init__('t1_servo_controller')
        self.t1_camera_direction_subscription = self.create_subscription(TowerData, 't1_camera_direction', self.t1_camera_direction_callback, 10)
        self.t1_camera_direction_subscription  # prevent unused variable warning
        ...
        self.publisher_ = self.create_publisher(TowerData, 't1_tower_data', 10)
        ...
        self.change_qr = False # if t2_change_qr, then True
    ...
    def t2_tower_data_callback(self, msg):
        #t2 will change_qr if both qr_ids are the same
        if msg.change_qr_notification == True:
            self.change_qr = True

    # set t1_camera_direction publish rate to change servo speed
    def t1_camera_direction_callback(self, msg):
...
def main(args=None):
    print('Publishing t1_tower_data...')
    rclpy.init(args=args)
    t1_servo_controller = T1ServoController()
    rclpy.spin(t1_servo_controller)

    # Destroy the node explicitly
    # (optional - otherwise it will be done automatically
    # when the garbage collector destroys the node object)
    t1_servo_controller.destroy_node()
    pwm.exit_PCA9685()
    rclpy.shutdown()
