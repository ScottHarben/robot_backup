...
class T1DirectionPublisher(Node):

    def __init__(self):
        super().__init__('t1_direction_publisher')
        self.t1_dist_subscription = self.create_subscription(TowerData, 't1_dist', self.t1_dist_callback, 10)
        self.t1_dist_subscription  # prevent unused variable warning

        self.publisher_ = self.create_publisher(TowerData, 't1_camera_direction', 10)
...
def main(args=None):
    print('Publishing t1_camera_direction...')
    rclpy.init(args=args)
    t1_direction_publisher = T1DirectionPublisher()
    try:
        rclpy.spin(t1_direction_publisher)
    except KeyboardInterrupt:
        t1_direction_publisher.vs.stop()
        print('USER STOPPED')
    ...
    t1_direction_publisher.destroy_node()
    rclpy.shutdown()
